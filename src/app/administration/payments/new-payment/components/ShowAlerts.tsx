...
const ShowAlerts = ({formOptions} : {formOptions: optionsForm}) => {
    ...
    return (
        <Group mb={'1rem'}>
            {paymentsResponse.errors.paymentTypes && <ErrorAlert errorMessage={'Error obteniendo tipos de pago'}/>}
            {playersResponse.errors.players && <ErrorAlert errorMessage={'Error obteniendo jugadores'}/>}
        </Group>
    )
}
